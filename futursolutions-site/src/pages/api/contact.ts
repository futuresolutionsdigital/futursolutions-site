	...
	if (webhookUrl) {
		try {
			...
			if (!res.ok) {
				...
				return json(502, { ok: false, message: 'We could not send your message. Please try again or email us directly.' });
			}
		} catch (err) {
		...
		console.log('Contact inquiry (no GHL_WEBHOOK_URL configured):', JSON.stringify(lead));
	}
